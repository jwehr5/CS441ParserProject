...
(define (parse textfile)
  ...
  ;Convert each line of the file into a string and put it into a list.
  ...
  (define (program? file)
    ...
    ;An expr may begin with an id, num, or an opening parenthesis.
    (define (expr? file linenumber)
      (if (empty? file)
          ...
          (cond
            ...
            [(number? (string->number(first (num? file)))) (etail? (rest (num? file)) linenumber) ]
    ...
    ;Since we have already checked the line number go ahead and pass the rest of our input to the stmt? function.
    ...
    #|
    linelist? is called recursively. At the start of every line we should see a linenumber.
    If we don't see a linenumber then the only other thing we should see is $$ which means we are at the end of the program.
    |# 
    (define (linelist? file)
      ;Always check to make sure that our list of tokens is not empty
      (if (empty? file)
          (error "Missing end of input $$")
          (cond
            ...
            In addition, grab the line number so we can pass it around to the other functions and easily state what line we are on if there's an error.
            |#
            ...
            [else (error "Syntax Error: Missing or invalid Line Number")])))
  ...
  ;If the scanning process was successful, then parse the file.
  (if(equal? (scan-tokens (file-to-lines)) #t)
      (program? (file-to-tokens (file-to-lines)))
      (error "File is unable to be parsed."))
